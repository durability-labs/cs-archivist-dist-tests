...
namespace ArchivistNetDeployer
{
    public class Configuration
    {
        ...
        [Uniform("deploy-file", "df", "DEPLOYFILE", false, "Output deployment JSON file that will be written. Defaults to 'archivist-deployment.json'.")]
        public string DeployFile { get; set; } = "archivist-deployment.json";

        [Uniform("archivist-local-repo", "cr", "ARCHIVISTLOCALREPOPATH", false, "If set, instead of using the default Archivist docker image, the local repository will be used to build an image. " +
        ...
        public string ArchivistLocalRepoPath { get; set; } = string.Empty;

        [Uniform("nodes", "n", "NODES", true, "Number of Archivist nodes to be created.")]
        public int? NumberOfArchivistNodes { get; set; }

        [Uniform("validators", "v", "VALIDATORS", true, "Number of Archivist nodes that will be validating.")]
        public int? NumberOfValidators { get; set; }

        [Uniform("storage-quota", "sq", "STORAGEQUOTA", true, "Storage quota in megabytes used by each Archivist node.")]
        public int? StorageQuota { get; set; }

        [Uniform("log-level", "l", "LOGLEVEL", true, "Log level used by each Archivist node. [Trace, Debug*, Info, Warn, Error]")]
        public ArchivistLogLevel ArchivistLogLevel { get; set; } = ArchivistLogLevel.Debug;
        
        [Uniform("log-level-libp2p", "lp2p", "LOGLEVELLIBP2P", true, "Log level for all libp2p topics. [Trace, Debug, Info, Warn*, Error]")]
        public ArchivistLogLevel Libp2pLogLevel { get; set; } = ArchivistLogLevel.Warn;

        [Uniform("log-level-discv5", "ldv5", "LOGLEVELDISCV5", true, "Log level for all discv5 topics. [Trace, Debug, Info, Warn*, Error]")]
        public ArchivistLogLevel Discv5LogLevel { get; set; } = ArchivistLogLevel.Warn;
        ...
        [Uniform("test-tokens", "tt", "TESTTOKENS", false, "Initial amount of test-tokens minted for each Archivist node.")]
        ...
        public List<string> Validate()
        {
            ...
            if (NumberOfValidators > NumberOfArchivistNodes)
            {
                errors.Add($"{nameof(NumberOfValidators)} ({NumberOfValidators}) may not be greater than {nameof(NumberOfArchivistNodes)} ({NumberOfArchivistNodes}).");
            }
            ...
            if (IsPublicTestNet)
            {
                if (NumberOfArchivistNodes > 0)
                {
                    if (PublicDiscPorts.Split(",").Length != NumberOfArchivistNodes) errors.Add("Number of public discovery-ports provided does not match number of archivist nodes.");
                    if (PublicListenPorts.Split(",").Length != NumberOfArchivistNodes) errors.Add("Number of public listen-ports provided does not match number of archivist nodes.");
                }
