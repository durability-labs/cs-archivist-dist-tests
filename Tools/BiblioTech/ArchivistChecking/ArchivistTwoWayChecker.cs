...
namespace BiblioTech.ArchivistChecking
{
    public interface ICheckResponseHandler
    {
        ...
        Task ArchivistUnavailable();

        Task ToAdminChannel(string msg);
    }

    public class ArchivistTwoWayChecker
    {
        ...
        public ArchivistTwoWayChecker(ILog log, Configuration config, CheckRepo repo, ArchivistWrapper archivistWrapper)
        {
            this.log = log;
            this.config = config;
            this.repo = repo;
            this.archivistWrapper = archivistWrapper;
        }
        ...
        public async Task StartDownloadCheck(ICheckResponseHandler handler, ulong userId)
        {
            ...
            if (cid == null)
            {
                await handler.ArchivistUnavailable();
                return;
            }
        ...
        private bool? IsContentCorrect(ICheckResponseHandler handler, TransferCheck check, string receivedCid)
        {
            try
            {
                var content = archivistWrapper.OnArchivist(node =>
                {
