...
namespace ContinuousTests
{
    public class StartupChecker
    {
        ...
        public void Check()
        {
            var log = FixtureLog.Create(new LogConfig(config.LogPath), DateTime.UtcNow, config.ArchivistDeployment.Id,
                "StartupChecks");
            ...
            log.Log("Contacting Archivist nodes...");
            CheckArchivistNodes(log, config);
            log.Log("All OK.");
        }
        ...
        private void IncludeDeploymentConfiguration(ILog log)
        {
            ...
            //var deployment = config.ArchivistDeployment;
            //var workflow = entryPoint.Tools.CreateWorkflow();
            //foreach (var instance in deployment.ArchivistInstances)
            //{
            //    foreach (var container in instance.Pod.Containers)
            //    {
            //        var podInfo = workflow.GetPodInfo(container);
            //        log.Log($"Archivist environment variables for '{container.Name}':");
            //        log.Log(
            ...
            //        var archivistVars = container.Recipe.EnvVars;
            //        foreach (var vars in archivistVars) log.Log(vars.ToString());
            //        log.Log("");
            //    }
            //}
        ...
        private void CheckArchivistNodes(ILog log, Configuration config)
        {
            throw new NotImplementedException();

            //var nodes = entryPoint.CreateInterface()
            //    .WrapArchivistContainers(config.ArchivistDeployment.ArchivistInstances.Select(i => i.Pod).ToArray());
            //var pass = true;
            ...
            //if (!pass)
            //{
            //    throw new Exception("Not all archivist nodes responded.");
            //}
        }

        private bool EnsureOnline(BaseLog log, IArchivistNode n)
        {
            try
            {
                ...
                log.Log($"Archivist version: '{info.Version.Version}' revision: '{info.Version.Revision}'");
        ...
        private void CheckRequiredNumberOfNodes(ContinuousTest[] tests, List<string> errors)
        {
            foreach (var test in tests)
            {
                if (test.RequiredNumberOfNodes != -1)
                {
                    ...
                    else if (test.RequiredNumberOfNodes > config.ArchivistDeployment.ArchivistInstances.Length)
                    {
                        errors.Add(
                            $"Test '{test.Name}' requires {test.RequiredNumberOfNodes} nodes. Deployment only has {config.ArchivistDeployment.ArchivistInstances.Length}");
                    }
                }
            }
        }
