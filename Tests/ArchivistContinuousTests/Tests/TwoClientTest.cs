...
namespace ContinuousTests.Tests
{
    ...
    {
        private const string BytesStoredMetric = "archivist_repostore_bytes_used";
        ...
        private void LogRepoStore(IArchivistNode archivistNode)
        {
            //var response = archivistNode.GetDebugRepoStore();
            //Log.Log($"{archivistNode.GetName()} has {string.Join(",", response.Select(r => r.cid))}");
        }

        private void LogStoredBytes(IArchivistNode node)
        {
        ...
        private void LogBlockExchangeStatus(IArchivistNode archivistNode, string msg)
        {
            //var response = archivistNode.GetDebugBlockExchange();
            //Log.Log($"{archivistNode.GetName()} {msg}: {JsonConvert.SerializeObject(response)}");
        }
    }
}
