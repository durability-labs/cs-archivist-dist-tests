...
namespace ContinuousTests
{
    ...
    public abstract class ContinuousTest
    {
        ...
        public void Initialize(IArchivistNode[] nodes, ILog log, IFileManager fileManager, Configuration configuration, CancellationToken cancelToken)
        {
            Nodes = nodes;
            Log = log;
        ...
        public IArchivistNode[] Nodes { get; private set; } = null!;
        ...
        {
            if (Configuration.ArchivistDeployment.PrometheusContainer == null) throw new Exception("Expected prometheus to be part of Archivist deployment.");

            var entryPointFactory = new EntryPointFactory();
            var entryPoint = entryPointFactory.CreateEntryPoint(Configuration.KubeConfigFile, Configuration.DataPath, Configuration.ArchivistDeployment.Metadata.KubeNamespace, Log);
            return entryPoint.CreateInterface().WrapMetricsCollector(Configuration.ArchivistDeployment.PrometheusContainer, target);
        }
