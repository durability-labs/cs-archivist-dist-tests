    ...
    public class SingleTestRun
    {
            ...
            StartupChecker startupChecker, CancellationToken cancelToken, string deployId)
        {
            ...
            entryPoint = entryPointFactory.CreateEntryPoint(config.KubeConfigFile, config.DataPath,
                config.ArchivistDeployment.Metadata.KubeNamespace, fixtureLog);
        ...
        private void DownloadContainerLogs(DateTime testStart)
        {
            ...
            {
                effectiveStart = config.ArchivistDeployment.Metadata.StartUtc.Subtract(TimeSpan.FromSeconds(30));
            }
        ...
        private IArchivistNode[] CreateRandomNodes()
        {
            var instances = SelectRandomInstance();
            fixtureLog.Log("Selected nodes: " + string.Join(",", instances.Select(c => c.Name)));
            return entryPoint.CreateInterface().WrapArchivistContainers(instances).ToArray();
        }

        private IArchivistInstance[] SelectRandomInstance()
        {
            var number = handle.Test.RequiredNumberOfNodes;
            var instances = config.ArchivistDeployment.ArchivistInstances.ToList();
            if (number == -1) return instances.ToArray();

            var result = new IArchivistInstance[number];
