...
namespace ArchivistTests.ScalabilityTests;

[TestFixture]
public class ScalabilityTests : ArchivistDistTest
{
    /// <summary>
    ...
    {
        var logLevel = ArchivistLogLevel.Trace;

        var bootstrap = StartArchivist(s => s.WithLogLevel(logLevel));
        var nodes = StartArchivist(numberOfNodes - 1, s => s
    ...
    {
        var logLevel = ArchivistLogLevel.Info;
    ...
    {
        public NodeFilePair(IArchivistNode node, TrackedFile file, ContentId cid)
        {
            Node = node;
            File = file;
            Cid = cid;
