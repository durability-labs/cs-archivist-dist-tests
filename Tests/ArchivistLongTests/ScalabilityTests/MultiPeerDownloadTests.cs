...
namespace ArchivistTests.ScalabilityTests
{
    ...
    public class MultiPeerDownloadTests : AutoBootstrapDistTest
    {
        ...
        {
            var hosts = StartArchivist(numberOfHosts, s => s.WithName("host").WithLogLevel(ArchivistLogLevel.Trace));
            ...
            var blockCids = uploadLog
                ...
                .Select(s =>
                {
                    var line = ArchivistLogLine.Parse(s)!;
            ...
            var client = StartArchivist(s => s.WithName("client").WithLogLevel(ArchivistLogLevel.Trace));
