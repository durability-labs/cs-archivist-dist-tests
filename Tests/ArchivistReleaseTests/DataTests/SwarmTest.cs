...
namespace ArchivistReleaseTests.DataTests
{
    namespace SwarmTests
    {
        ...
        public class SwarmTests : AutoBootstrapDistTest
        {
            ...
            private Task StartDownload(IArchivistNode node, SwarmTestNetworkFile[] files)
            {
                return Task.Run(() =>
                {
            ...
            private Task StartStreamlessDownload(IArchivistNode node, SwarmTestNetworkFile[] files)
            {
                return Task.Run(() =>
                {
                    var remaining = files.ToList();
            ...
            {
                public SwarmTestNetworkFile(IArchivistNode uploader, ByteSize originalSize, TrackedFile original, ContentId cid)
                {
                    Uploader = uploader;
                ...
                public IArchivistNode Uploader { get; }
