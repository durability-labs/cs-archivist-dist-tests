...
{
    public class ArchivistDiscordBotPlugin : IProjectPlugin, IHasLogPrefix, IHasMetadata
    {
        private const string ExpectedStartupMessage = "Debug option is set. Discord connection disabled!";
        private readonly IPluginTools tools;

        public ArchivistDiscordBotPlugin(IPluginTools tools)
        {
            this.tools = tools;
        }
        ...
        {
            tools.GetLog().Log($"Archivist DiscordBot (BiblioTech) loaded.");
        }

        public void AddMetadata(IAddMetadata metadata)
        {
            metadata.Add("archivistdiscordbotid", new DiscordBotContainerRecipe().Image);
        }
