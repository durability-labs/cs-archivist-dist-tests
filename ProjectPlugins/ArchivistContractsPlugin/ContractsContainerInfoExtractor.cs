...
namespace ArchivistContractsPlugin
{
    public class ContractsContainerInfoExtractor
    {
        ...
        public (string, string) ExtractMarketplaceAbiAndByteCode()
        {
            ...
            if (string.IsNullOrEmpty(abi)) throw new InvalidOperationException("Unable to fetch marketplace artifacts from archivist-contracts node. Test infra failure.");
        ...
        {
            var json = workflow.ExecuteCommand(container, "cat", ArchivistContractsContainerRecipe.DeployedAddressesFilename);
            json = json.Replace("#", "_");
        ...
        {
            var json = workflow.ExecuteCommand(container, "cat", ArchivistContractsContainerRecipe.MarketplaceArtifactFilename);
